 ...
 */
function httpProbe(port, timeoutMs, checkBody) {
    return new Promise((resolve) => {
        const req = (0, http_1.get)({ host: "127.0.0.1", port, path: "/", timeout: timeoutMs }, (res) => {
            if (!checkBody) {
                req.destroy();
                resolve(true);
                return;
...
function probeNode(timeoutMs = 8000) {
    return new Promise((resolve) => {
        const child = (0, child_process_1.spawn)("node --version", {
            shell: true,
            windowsHide: true,
            stdio: "ignore",
        });
...
function detectDshCommand() {
    ...
    candidates.push((0, path_1.join)(appData, "npm", "dsh.cmd"));
    const localAppData = process.env.LOCALAPPDATA ?? (0, path_1.join)((0, os_1.homedir)(), "AppData", "Local");
    const npxRoot = (0, path_1.join)(localAppData, "npm-cache", "_npx");
    if ((0, fs_1.existsSync)(npxRoot)) {
        try {
            for (const sub of (0, fs_1.readdirSync)(npxRoot)) {
                candidates.push((0, path_1.join)(npxRoot, sub, "node_modules", ".bin", "dsh.cmd"));
            }
        }
        catch {
            // 目录不可读时忽略，走下一个候选
...
function vaultHome(vaultPath) {
    const localAppData = process.env.LOCALAPPDATA ?? (0, path_1.join)((0, os_1.homedir)(), "AppData", "Local");
    return (0, path_1.join)(localAppData, "dsh-obsidian", hashPath(vaultPath).toString(16));
}
/**
 * 首次启动时把主 .dsh 的凭据和设置继承到库专属目录，
 * 免去每个库重复配置 API 密钥与搜索网关。
 */
function inheritMainConfig(vaultHomePath) {
    if ((0, fs_1.existsSync)((0, path_1.join)(vaultHomePath, "settings.yaml")))
        return; // 已初始化过，不覆盖
    const mainHome = process.env.DSH_HOME ?? (0, path_1.join)((0, os_1.homedir)(), ".dsh");
    (0, fs_1.mkdirSync)(vaultHomePath, { recursive: true });
    for (const file of [".credentials.yaml", "settings.yaml"]) {
        const source = (0, path_1.join)(mainHome, file);
        if ((0, fs_1.existsSync)(source)) {
            try {
                (0, fs_1.copyFileSync)(source, (0, path_1.join)(vaultHomePath, file));
            }
            catch {
...
function seedWorkspace(vaultHomePath, vaultPath) {
    const storageFile = (0, path_1.join)(vaultHomePath, "storages", "workspace.json");
    let canonical;
    try {
        canonical = (0, fs_1.realpathSync)(vaultPath);
    }
    catch {
    ...
    if ((0, fs_1.existsSync)(storageFile)) {
        try {
            const parsed = JSON.parse((0, fs_1.readFileSync)(storageFile, "utf8"));
    ...
    data.tables.workspaces[id] = {
        path: canonical,
        title: (0, path_1.basename)(canonical),
        sessionIds: [],
        createdAt: now,
        updatedAt: now,
    };
    try {
        (0, fs_1.mkdirSync)((0, path_1.dirname)(storageFile), { recursive: true });
        (0, fs_1.writeFileSync)(storageFile, JSON.stringify(data), "utf8");
    }
    catch {
...
function spawnDsh(bootCommand, vaultPath) {
    const child = (0, child_process_1.spawn)(bootCommand, {
        shell: true,
        cwd: vaultPath,
        windowsHide: true,
        stdio: "ignore",
...
class DshView extends obsidian_1.ItemView {
    ...
    async onOpen() {
        ...
        try {
            ...
            new obsidian_1.Notice(`DSH 启动失败：${message}`, 10000);
        }
    }
}
class DshPlugin extends obsidian_1.Plugin {
    manager;
    statusBar;
    async onload() {
        await this.loadSettings();
        this.manager = new InstanceManager(() => this.settings, () => this.saveSettings());
        ...
        (0, obsidian_1.addIcon)("dsh-logo", `<path d="${FISH_LOGO_PATH_D}" fill="currentColor" transform="scale(2)"/>`);
    ...
    async openView() {
        ...
        if (!leaf) {
            new obsidian_1.Notice("无法创建 DSH 面板，请尝试重启 Obsidian");
            return;
        }
        if (!existing) {
    ...
    updateStatusBar(text) {
        if (this.statusBar)
            this.statusBar.setText(`DSH: ${text}`);
    }
    async loadSettings() {
...
exports.default = DshPlugin;
class DshSettingTab extends obsidian_1.PluginSettingTab {
    plugin;
    ...
    display() {
        ...
        new obsidian_1.Setting(containerEl)
            .setName("dsh 可执行文件路径")
            .setDesc("留空则自动探测（npm 全局安装 → npx 缓存 → 在线 npx 安装）。" +
        ...
        new obsidian_1.Setting(containerEl)
            .setName("基础端口")
            .setDesc("每个库占用一个端口（从基础端口往上找空闲）。默认 3090，避开桌面版 DSH 常用的 3080。")
            .addText((text) => text
        ...
        new obsidian_1.Setting(containerEl)
            ...
            .addToggle((toggle) => toggle.setValue(this.plugin.settings.stopOnUnload).onChange(async (value) => {
